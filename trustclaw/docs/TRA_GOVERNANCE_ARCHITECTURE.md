...
规范要求：**同一协调器 session key** 下，OpenClaw 侧须通过 `resolveCoordinatorAgentPack({ bindLock: true })` 固定 Business Pack，避免侧边栏换 agent 导致 **问责主体漂移**（`trustclaw/runtime/coordinator/session-pack-coordinator.ts`）。存储键由 `resolveCoordinatorSessionKey` 规范：bare `session_id` + `openclaw_agent_id` → `agent:<agentId>:<session>`；Control UI 已传 `agent:` 前缀的完整 `sessionKey`。
