...
export function listDomainAgents(
  ...
  const dbPath = resolvePtdsDbPath(overrides, env);
  ...
  try {
    ...
    const rows = db
      .prepare(
        `SELECT agent_id, agent_name, domain, subdomain, region, insurance_type, enabled, ptds_scopes, ptds_write, pack_id, pack_version, registered_at
         FROM domain_agents
         ${where}
