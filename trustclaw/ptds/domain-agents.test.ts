  ...
  try {
    ...
    db.prepare(
      `INSERT INTO domain_agents (agent_id, agent_name, domain, enabled, pack_id)
       VALUES (?, ?, ?, ?, ?)`,
    ).run("op-001", "Outpatient 1", "outpatient", "partial", "ptds-outpatient");
    db.prepare(
      `INSERT INTO domain_agents (agent_id, agent_name, domain, enabled, pack_id)
       VALUES (?, ?, ?, ?, ?)`,
    ).run("op-002", "Outpatient 2", "outpatient", "false", "ptds-outpatient");
    db.prepare(
      `INSERT INTO domain_agents (agent_id, agent_name, domain, enabled, pack_id)
       VALUES (?, ?, ?, ?, ?)`,
    ).run("drg-001", "Drug 1", "pharmacy", "partial", "ptds-pharmacy");
  } finally {
    db.close();
  }
}
...
describe("listDomainAgents", () => {
  ...
  it("lists agents with filters and summary", () => {
    ...
    try {
      ...
      const pack = listDomainAgents({ dbPath }, { pack_id: "ptds-outpatient" });
      expect(pack.agents).toHaveLength(2);
      expect(pack.summary.by_pack["ptds-outpatient"]).toBe(2);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
