...
import { createTrustclawPtdsWriteToolFactory } from "./ptds-write-tool.js";
...
describe("trustclaw_ptds_write tool", () => {
  it("writes personal data and returns row counts in tool details", async () => {
    ...
    try {
      ...
      const factory = createTrustclawPtdsWriteToolFactory(
        { dbPath },
      ...
      expect(text).toContain("已写入 PTDS");
      const details = (result as { details?: { trustclaw?: { personal_write?: { status: string } } } })
        .details;
