...
1. `trustclaw/GETTING_STARTED.md` — dev 启动、端口、模型、PTDS Console 布局  
2. `trustclaw/PLAN.md` — 产品阶段与任务矩阵  
3. `trustclaw/DECISIONS.md` — **逐条审核**；`pending` 项不得实现  
4. `trustclaw/OPENCLAW_REUSE.md` — Inherit / Extend / Build 映射  
5. `trustclaw/PRODUCT_SPEC.md` — 冻结 API 与 JSON 合约  
6. 本节 [合规审查与数据审计](#合规审查与数据审计compliance-review) — 内部/外部数据、模型边界、审计 **Must** 标准  
7. `trustclaw/ROADMAP.md` — 5 天 Sprint  
8. Root `AGENTS.md` — 构建、测试、Git 政策  
...
| **C · 模型可见载荷** | Text2SQL 生成的 SQL、schema 摘要、握手矩阵、C3-PO 挂载 profile **摘要**（非全表 dump） | 仅当次 prompt / Runtime Context | A 类经 tool consent；B 类经 import consent；禁止 spec 外字段 | `TEXT2SQL_GEN`、`AGENT_DECISION` 等 |
...
4. 新增模型-facing 字段或 prompt 注入 → 更新 `PRODUCT_SPEC.md` + 本节 + colocated test。
...
| `ui_six_panels` | Chrome A–F + C + D + E | 进化 | Task 501–503 + Panel F |
...
- **下一轮建议（R7）**：Task **401** evidence ledger 真链；consent/import 事件 Panel D 可视化；`prescription_context` 从 init 采集或 DECISIONS 明确 demo 契约；publisher 签名校验（若 SPEC 批准）。
...
| Demo UI | `trustclaw/ui/` | 501–503 △ | Vite SPA + plugin static `/trustclaw/*` |
| PTDS Console shell | `ui/src/ui/views/trustclaw-ptds-workbench.ts` | 501–503 △ | Control UI 嵌入；`trustclaw-ptds-bridge.ts` |
...
## PTDS Console（Task 501–503）

Dev 一键：`pnpm trustclaw:setup && pnpm trustclaw:dev` → Gateway `:19001` + Vite `:5174/trustclaw/`。

| 区域 | 路径 / 行为 |
| --- | --- |
| **C · Chat** | Control UI 原生 Chat（`trustclaw_ptds_query` 工具） |
| **A + F + B · 左栏** | iframe `/trustclaw/?embed=left` — init + **合规标准导入** + 数据浏览 |
| **D + E · 右栏** | iframe `/trustclaw/?embed=right` — 审计 + 账本（Runtime Context `postMessage`） |
