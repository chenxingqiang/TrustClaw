...
export type PtdsClinicalDiagnosisSummary = {
  icd10_code: string;
  diagnosis_name: string;
};

export type PtdsMedicationSummary = {
  drug_name: string;
  termination_reason: string | null;
};

/** Human-readable private data fields shown in consent prompts. */
export const PTDS_PRIVATE_DATA_FIELD_LABELS: Record<string, { en: string; "zh-CN": string }> = {
...
export type PtdsHealthProfileSummary = {
  mounted: boolean;
  patient_name: string;
  gender: "男" | "女";
  age_years: number;
  ...
  diagnoses: PtdsClinicalDiagnosisSummary[];
  medications: PtdsMedicationSummary[];
  snapshot: Glp1CheckSnapshot | null;
  /** Keys from PTDS_PRIVATE_DATA_FIELD_LABELS present in this profile. */
...
function readProfileFromDb(db: DatabaseSync): Omit<PtdsHealthProfileSummary, "mounted"> | null {
  ...
  const profile = db
    .prepare(
      `SELECT name, birth_date, biological_sex FROM user_profile WHERE user_id = ? LIMIT 1`,
    )
  ...
  const diagnoses = db
    ...
    .all() as PtdsClinicalDiagnosisSummary[];
...
export function buildPtdsHealthProfileSummary(
  dbPathOrOverrides?: string | PtdsPathOverrides,
  env: NodeJS.ProcessEnv = process.env,
): PtdsHealthProfileSummary {
  const dbPath =
...
export function formatPrivateDataFieldLabels(
  ...
  return fieldKeys.map((key) => PTDS_PRIVATE_DATA_FIELD_LABELS[key]?.[locale] ?? key);
}
