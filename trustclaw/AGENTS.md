...
2. **任务 ID**：从 [Loop manifest](#v1-闭环烟雾清单loop-manifest) 与 [当前轮次笔记](#当前轮次笔记由-agent-持续追加) 取主攻项；不得从其他文档自行开新 loop
...
本仓库 V1 交付**没有「做完就停」**。每一轮闭环的目标：**感知现状 → 选定瓶颈 → 最小落地 → 用证据验证 → 写回本文件 → commit → 进入下一轮**。
...
2. **路径**：是否落在 `DECISIONS.md` 开放项与本文件 Loop manifest？`DECISIONS.md` 是否有未闭合阻塞项？
...
**目标**：弄清当前 Loop manifest 完成度、契约覆盖、测试红项，以及 V1 闭环（Init → Chat → … → Dashboard）在哪一段断裂。

**典型动作**：

- 读契约：`DECISIONS.md` 开放项 + 本文件 Loop manifest
...
**目标**：根据感知结果排序，选定**单一**主攻项（例如：闭合 `POST /api/agent/chat` 骨架）。
...
| Verify    | DoD + Loop manifest 验收                                  | 见第 4 层                   |
...
V1 **已于 2026-07-05 闭合**（见 `DECISIONS.md`）。Loop 仍对照以下五项做回归：

- [x] **Runnable** — `pnpm trustclaw:dev` 或 `openclaw gateway run` + trustclaw 插件；仅本地 SQLite
- [x] **Demo Ready** — Chrome 稳定；Reset 清空 PTDS + audit + ledger
- [x] **Auditable** — TEXT2SQL / QUERY / RULE / DECISION / LEDGER 五步 UI 可见
- [x] **Evidence Generated** — 每次 Chat 产生带 `previous_evidence_hash` 的 receipt（Task 401）
- [x] **No Blocking** — 无 V1 范围外功能；无阻断 Bug

### V1 闭环烟雾清单（Loop manifest）

下表为 Agent 每轮 **Verify** 或合并后 **Perceive** 用的产品路径（**不**替代单元测试）。
...
[ ] 1. 感知：读 DECISIONS + Loop manifest；跑相关 trustclaw/*.test.ts；记录失败与缺口
[ ] 2. 策略：只选 1 个主攻项；对照 P0→P3 分层与 OpenClaw-first
...
| 感知          | `DECISIONS.md`、`OPENCLAW_REUSE.md`                           |
| 策略          | Loop manifest、`DECISIONS.md` deferred 边界                                  |
...
- **V1 基线（2026-07-05）**：`DECISIONS.md` D1–D24；闭环 Init → Chat → … → Console 已交付；`PLAN.md` / `PRODUCT.md` / `ROADMAP.md` 已退役，Loop 只读本文件 + `DECISIONS.md`。
- **R1–R12（2026-07-04→05）**：PTDS v1.1 + init/reset → Text2SQL + 规则引擎 → Chat 管线 + 五步审计 → 证据链 → Panels A–F + 合规 import/consent → 领域赋权 + Panel B 订阅血缘。
- **R13–R16（2026-07-05）**：DoD 自动化（五步审计闸门、`dod-reset-demo`）；Panel C 赋权 + evidence `agent_pack_id`；Panel B 血缘/订阅；`pnpm build` + dev 烟雾绿。
- **R17（2026-07-05，文档整理）**：精简 `DECISIONS.md`；删除 `PLAN`/`PRODUCT`/`ROADMAP`；根文档与 `VISION.md` 对齐 V1 已交付；`trustclaw/README.md` 去 stale 进度。
- **下一轮建议（V2）**：D5 频道桥接 spike；D13 品牌化；D24 全量 `domain_agents` 导入；Chrome 双遍 reset demo 人工回归（可选）。
...
| GLP-1 prompts        | `trustclaw/agents/glp1/`                                               | ✓      | skills-like prompts                   |
