...
2. **Bottom — Logic Agent registry:** rows from `domain_agents` — 领域目录, filter by `pack_id` / `enabled`.
...
- **Pack list is Read-only over HTTP** — Create/Update/Delete packs via repo files until Phase 4 authoring API.
...
| `GET /api/tra/agent-packs`                             | GET              | P3 pack registry                  |
